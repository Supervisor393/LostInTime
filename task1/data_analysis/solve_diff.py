...
def process_csv_and_output(csv_file_path: str):
    """处理每个CSV文件并输出结果到终端，根据 is_recognized 分组"""
    # 读取CSV文件
    df = pd.read_csv(csv_file_path)
    
    # 分别处理 is_recognized = False 和 is_recognized = True 的数据
    for recognized_value in [False, True]:
        df_group = df[df['is_recognized'] == recognized_value]
        
        # 总条数（不包含表头）
        total_count = len(df_group)
        
        # 计算 diff 为空的行数
        diff_is_nan = df_group['diff'].isna().sum()
        
        # 有效数据条数 = 总条数 - diff为空的条数
        valid_count = total_count - diff_is_nan
        
        # 计算 diff 小于等于 0 的均值和数据条数
        diff_leq_0 = df_group[df_group['diff'] <= 0]['diff']
        mean_diff_leq_0 = diff_leq_0.mean() if len(diff_leq_0) > 0 else None
        count_diff_leq_0 = len(diff_leq_0)
        
        # 计算 diff 大于等于 0 的均值和数据条数
        diff_geq_0 = df_group[df_group['diff'] >= 0]['diff']
        mean_diff_geq_0 = diff_geq_0.mean() if len(diff_geq_0) > 0 else None
        count_diff_geq_0 = len(diff_geq_0)
        
        # 计算所有有效数据的 diff 的绝对值的均值
        valid_diff = df_group['diff'].dropna().abs()  # 只考虑有效数据并取绝对值
        mean_abs_diff = valid_diff.mean() if len(valid_diff) > 0 else None
        
        # 输出统计信息
        recognized_str = "True" if recognized_value else "False"
        print(f"文件: {csv_file_path} (is_recognized = {recognized_str})")
...
if __name__ == "__main__":
    ...
    input_folder = '/data/user/jzt/crd/audioLLM/biaozhuncha/qwen/2'  # 你的输入文件夹路径
